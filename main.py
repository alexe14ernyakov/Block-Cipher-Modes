...
def task4():
    ...
    print(f'Encrypted text with ECB: {encrypted_text4_1}; length = {len(plaintext)}')
    decrypted_text4_1 = bc4_1.decrypt(encrypted_text4_1)
    print(f'Decrypted text with ECB: {decrypted_text4_1}; length = {len(plaintext)}\n')

    bc4_2 = BlockCipher(key4, Mode.CBC)
    print(f'Plaintext:               {plaintext}; length = {len(plaintext)}')
    encrypted_text4_2 = bc4_2.encrypt(plaintext, custom_iv)
    print(f'Encrypted text with CBC: {encrypted_text4_2}; length = {len(plaintext)}')
    decrypted_text4_2 = bc4_2.decrypt(encrypted_text4_2, custom_iv)
    print(f'Decrypted text with CBC: {decrypted_text4_2}; length = {len(plaintext)}\n')

    bc4_3 = BlockCipher(key4, Mode.CFB)
    print(f'Plaintext:               {plaintext}; length = {len(plaintext)}')
    encrypted_text4_3 = bc4_3.encrypt(plaintext, custom_iv)
    print(f'Encrypted text with CFB: {encrypted_text4_3}; length = {len(plaintext)}')
    decrypted_text4_3 = bc4_3.decrypt(encrypted_text4_3, custom_iv)
    print(f'Decrypted text with CFB: {decrypted_text4_3}; length = {len(plaintext)}\n')

    bc4_4 = BlockCipher(key4, Mode.OFB)
    print(f'Plaintext:               {plaintext}; length = {len(plaintext)}')
    encrypted_text4_4 = bc4_4.encrypt(plaintext, custom_iv)
    print(f'Encrypted text with OFB: {encrypted_text4_4}; length = {len(plaintext)}')
    decrypted_text4_4 = bc4_4.decrypt(encrypted_text4_4, custom_iv)
    print(f'Decrypted text with OFB: {decrypted_text4_4}; length = {len(plaintext)}\n')

    bc4_5 = BlockCipher(key4, Mode.CTR)
    print(f'Plaintext:               {plaintext}; length = {len(plaintext)}')
    encrypted_text4_5 = bc4_5.encrypt(plaintext, custom_iv)
    print(f'Encrypted text with CTR: {encrypted_text4_5}; length = {len(plaintext)}')
    decrypted_text4_5 = bc4_5.decrypt(encrypted_text4_5, custom_iv)
    print(f'Decrypted text with CTR: {decrypted_text4_5}; length = {len(plaintext)}\n')
